...
def create_combined_count_plot(data_files, output_dir, color_map):
    ...
    plt.savefig(output_path, dpi=300)
    plt.close(fig)
    # print out the total number of each species at the end of the simulation
    print("\nFinal total particle counts per species:")
    for species, counts in all_species_counts.items():
        final_count = counts.iloc[-1]
        print(f"  - {species}: {final_count:.3e}")
